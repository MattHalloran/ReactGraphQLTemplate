...
export const resolvers = {
    Query: {
        businesses: async (_parent, _args, context, info) => {
            // Must be admin
    ...
    Mutation: {
        addBusiness: async(_parent, args, context, info) => {
            // Must be admin
            if(!context.req.isAdmin) return new CustomError(CODE.Unauthorized);
            return await context.prisma.business.create((new PrismaSelect(info).value), { data: { ...args.input } })
        },
        updateBusiness: async(_parent, args, context, info) => {
        ...
        deleteBusinesses: async(_parent, args, context, _info) => {
