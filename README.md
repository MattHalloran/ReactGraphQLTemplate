...
GraphQL is already typed, but it unfortunately doesn't play well with TypeScript's typing system. Instead of creating TypeScript types yourself (which is tedious), they can be generated by:  
1. Start project in local development (if ui is not runnable, that's fine. We just need a working server) - `docker-compose up -d`
2. `cd packages/ui` 
3. Generate schema types - `yarn gen-graphql-types`  
By default, `gen-graphql-types` points to `localhost:5000`. If you'd like to point to an external endpoint, the command should still work. If you'd like to point to a local schema file, that is also possible. See [this video](https://youtu.be/Tw_wn6XUfnU) for a guide. You can use `yarn download-schema` to download the schema at `localhost:5000` (or whatever you set it to).
